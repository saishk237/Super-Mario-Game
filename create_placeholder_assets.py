...
def create_sound_effects():
    ...
    jump_tone = np.sin(2 * np.pi * 440 * t) * 0.5
    # Make it stereo by stacking the array
    stereo_jump = np.vstack((jump_tone, jump_tone)).T
    jump_sound = pygame.sndarray.make_sound((stereo_jump * 32767).astype(np.int16))
    pygame.mixer.Sound.save(jump_sound, os.path.join('assets', 'sounds', 'jump.wav'))
    
    # Create a simple coin sound
    duration = 0.1
    t = np.linspace(0, duration, int(sample_rate * duration), False)
    coin_tone = np.sin(2 * np.pi * 880 * t) * 0.5
    # Make it stereo
    stereo_coin = np.vstack((coin_tone, coin_tone)).T
    coin_sound = pygame.sndarray.make_sound((stereo_coin * 32767).astype(np.int16))
    pygame.mixer.Sound.save(coin_sound, os.path.join('assets', 'sounds', 'coin.wav'))
    
    # Create a simple death sound
    duration = 0.5
    t = np.linspace(0, duration, int(sample_rate * duration), False)
    death_tone = np.sin(2 * np.pi * 220 * t) * 0.5
    # Make it stereo
    stereo_death = np.vstack((death_tone, death_tone)).T
    death_sound = pygame.sndarray.make_sound((stereo_death * 32767).astype(np.int16))
    pygame.mixer.Sound.save(death_sound, os.path.join('assets', 'sounds', 'death.wav'))
    ...
    for i in range(16):
        ...
        music[start:end] = np.sin(2 * np.pi * note * t[start:end]) * 0.3
    
    # Make it stereo
    stereo_music = np.vstack((music, music)).T
    bg_music = pygame.sndarray.make_sound((stereo_music * 32767).astype(np.int16))
    pygame.mixer.Sound.save(bg_music, os.path.join('assets', 'sounds', 'background_music.wav'))
